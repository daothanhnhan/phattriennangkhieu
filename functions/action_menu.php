...
class action_menu extends library{
    ...
    public function showMenu_byMultiLevel_mainMenuCarrot($list_menu,$valParent_menu = 0,$lang, $level = 0){
        ...
        if ($cate_child) {
            if ($level == 0) {
                echo '<ul class="list-inline menu_chinh">';
            } else {
                echo '<ul class="vk-menu-sub animated fadeIn">';
            }
            ...
            foreach ($cate_child as $key => $item) {
                ...
                if ($level == 0) {
                    if ($row[$this->nameColTitle_menuLanguage] == 'ĐÀO TẠO') {
                        echo '<a href="#" class="link_item_main_menu">'.$row[$this->nameColTitle_menuLanguage].'</a>';
                    } else {
